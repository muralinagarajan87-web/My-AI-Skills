...
function StatCard({ icon, label, value, sub, color, bgColor }) {
  return (
    <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '14px', bgcolor: 'white', height: '100%',
      transition: 'box-shadow 0.18s, transform 0.18s', '&:hover': { boxShadow: '0 4px 20px rgba(0,0,0,0.08)', transform: 'translateY(-2px)' } }}>
      <CardContent sx={{ p: 2.5, '&:last-child': { pb: 2.5 } }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box>
            <Typography sx={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.6, textTransform: 'uppercase', color: 'text.disabled', mb: 0.75 }}>{label}</Typography>
            <Typography sx={{ fontSize: 30, fontWeight: 800, color, lineHeight: 1.1 }}>{value}</Typography>
...
function SectionTitle({ children, sub }) {
  return (
    <Box sx={{ mb: 2 }}>
      <Typography sx={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.6, color: 'text.secondary' }}>{children}</Typography>
        ...
        return (
          <Tooltip key={i} title={`${d[labelKey]}: ${val}`} arrow>
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, cursor: 'default' }}>
              <Box sx={{ width: '100%', bgcolor: color, borderRadius: '3px 3px 0 0', height: `${Math.max(pct, 3)}%`, minHeight: 3,
                transition: 'height 0.4s ease', opacity: 0.85, '&:hover': { opacity: 1 } }} />
            </Box>
          </Tooltip>
        );
      })}
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
      {sorted.map(row => {
        ...
        const healthColor = health === 'good' ? { bg: '#dcfce7', bar: '#16a34a', text: '#15803d' }
          ...
          <Box key={row.priority} sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ width: 72, flexShrink: 0 }}>
              <Chip label={row.priority || 'None'} size="small"
                sx={{ height: 20, fontSize: 10, fontWeight: 700, bgcolor: healthColor.bg, color: healthColor.text, borderRadius: '4px', width: '100%' }} />
            </Box>
            <Box sx={{ flex: 1, bgcolor: '#f1f5f9', borderRadius: '6px', height: 14, overflow: 'hidden' }}>
              <Box sx={{ width: `${coveragePct}%`, height: '100%', bgcolor: healthColor.bar, borderRadius: '6px', transition: 'width 0.5s' }} />
            </Box>
            <Typography sx={{ fontSize: 11, fontWeight: 700, color: healthColor.text, minWidth: 36, textAlign: 'right' }}>{coveragePct}%</Typography>
...
export default function DashboardPage() {
  ...
  useEffect(() => { loadWorkspace(); loadTemplates(); loadTestRuns(); loadDashboard(); }, []);
  ...
  const loadMetrics = async (runId) => { try { const r = await reportAPI.getMetrics(runId); setMetrics(r.data); } catch (e) { console.error(e); } };
  const loadTestRuns = async () => { try { const r = await testRunAPI.getAll(); setTestRuns(r.data); } catch (e) { console.error(e); } };
  const loadDashboard = async () => { try { const r = await reportAPI.getDashboard(); setDashboard(r.data); } catch (e) { console.error(e); } };
  ...
  // Pre-release checklist items derived from dashboard data
  const checklist = [
  ...
  return (
    <Box sx={{ bgcolor: '#f4f6f9', minHeight: '100vh', pb: 6 }}>
      ...
      {/* ── HERO HEADER ──────────────────────────────────────────── */}
      <Box sx={{ background: 'linear-gradient(135deg, #1a3d2b 0%, #225038 60%, #2d6a4f 100%)',
        px: { xs: 3, md: 6 }, pt: 4, pb: 5, position: 'relative', overflow: 'hidden' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2, position: 'relative' }}>
          <Box>
            ...
            <Typography sx={{ fontSize: 32, fontWeight: 800, color: 'white', lineHeight: 1.15, mb: 0.5 }}>Dashboard</Typography>
            <Typography sx={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Test quality overview & release intelligence</Typography>
          </Box>
          <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'center' }}>
            <Button component={RouterLink} to="/test-runs" variant="contained" startIcon={<AddIcon />}
              sx={{ bgcolor: 'white', color: '#225038', fontWeight: 700, borderRadius: '10px', px: 2.5, fontSize: 13, boxShadow: 'none',
            ...
            <FormControl size="small">
              <Select value={selectedTemplateId} onChange={e => setSelectedTemplateId(e.target.value)} displayEmpty
                ...
                sx={{ bgcolor: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '10px', color: 'white', minWidth: 150, fontSize: 13,
            ...
            <Button variant="outlined" size="small" startIcon={<UploadFileIcon sx={{ fontSize: 16 }} />}
              ...
              sx={{ borderRadius: '10px', borderColor: 'rgba(255,255,255,0.3)', color: 'rgba(255,255,255,0.85)', fontSize: 13, px: 2,
                '&:hover': { borderColor: 'white', bgcolor: 'rgba(255,255,255,0.1)' } }}>Upload</Button>
            <Button variant="outlined" size="small" startIcon={<DownloadIcon sx={{ fontSize: 16 }} />}
              onClick={handleDownloadExcel}
              sx={{ borderRadius: '10px', borderColor: 'rgba(255,255,255,0.3)', color: 'rgba(255,255,255,0.85)', fontSize: 13, px: 2,
                '&:hover': { borderColor: 'white', bgcolor: 'rgba(255,255,255,0.1)' } }}>Export</Button>
          </Box>
        </Box>
      </Box>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, mt: -2 }}>
        ...
        {/* ── ROW 2: RELEASE READINESS + CHECKLIST + MILESTONE RISK ─ */}
        <Grid container spacing={3} sx={{ mb: 3 }}>

          {/* Release Readiness Gauge */}
          <Grid item xs={12} md={4}>
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                <SectionTitle sub="Weighted score: coverage x quality x critical tests">Release Readiness</SectionTitle>
              ...
              <CardContent sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  ...
                  <Chip label={`${checkDone}/${checkTotal}`} size="small"
                    sx={{ bgcolor: checkDone === checkTotal ? '#dcfce7' : '#fef3c7', color: checkDone === checkTotal ? '#15803d' : '#92400e', fontWeight: 700, fontSize: 11 }} />
                </Box>
                {/* Progress bar */}
                <Box sx={{ mb: 2.5 }}>
                  <Box sx={{ display: 'flex', height: 6, borderRadius: 3, bgcolor: '#f1f5f9', overflow: 'hidden', mb: 0.75 }}>
                    <Box sx={{ width: `${(checkDone / checkTotal) * 100}%`, bgcolor: checkDone === checkTotal ? '#16a34a' : '#d97706', transition: 'width 0.5s', borderRadius: 3 }} />
                  </Box>
                ...
                {checklist.map((item, i) => (
                  <Box key={i} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.25, mb: 1.5, '&:last-child': { mb: 0 } }}>
                    <Box sx={{ width: 18, height: 18, borderRadius: '50%', flexShrink: 0, mt: 0.1, display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <CardContent sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  ...
                  <Button component={RouterLink} to="/milestones" size="small" endIcon={<ArrowForwardIosIcon sx={{ fontSize: 10 }} />}
                    sx={{ fontSize: 11, color: '#225038', fontWeight: 600, minWidth: 0 }}>View all</Button>
                </Box>
                {!dashboard?.milestones || dashboard.milestones.length === 0 ? (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
                    <CheckCircleOutlineIcon sx={{ fontSize: 36, color: '#86efac', mb: 1 }} />
                    <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#16a34a' }}>No upcoming milestones</Typography>
                  ...
                  const urgencyStyle = urgency === 'high' ? { bg: '#fee2e2', color: '#dc2626', border: '#fca5a5' }
                    : urgency === 'med' ? { bg: '#fef3c7', color: '#d97706', border: '#fde047' }
                    : { bg: '#f0fdf4', color: '#16a34a', border: '#86efac' };
                  return (
                    <Box key={m.id} sx={{ mb: 2, p: 1.75, borderRadius: '10px', bgcolor: urgencyStyle.bg, border: '1px solid', borderColor: urgencyStyle.border, '&:last-child': { mb: 0 } }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <Chip label={`${daysLeft}d`} size="small"
                          sx={{ height: 18, fontSize: 10, fontWeight: 800, bgcolor: urgencyStyle.color, color: 'white', borderRadius: '4px', ml: 1, flexShrink: 0 }} />
                      </Box>
            ...
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                {!dashboard?.trend || dashboard.trend.length === 0 ? (
                  <Typography sx={{ fontSize: 12, color: 'text.disabled', py: 3, textAlign: 'center' }}>No test runs with results yet</Typography>
                ) : (
                  <>
                      ...
                      {dashboard.trend.map((run, i) => {
                        ...
                        return (
                          <Tooltip key={i} title={`${run.name}: ${rate}% (${run.passed}/${total})`} arrow>
                            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, cursor: 'default' }}>
                              <Box sx={{ width: '100%', bgcolor: color, borderRadius: '3px 3px 0 0', height: `${Math.max(rate, 4)}%`,
                                opacity: 0.85, '&:hover': { opacity: 1 }, transition: 'all 0.3s' }} />
                            </Box>
                          </Tooltip>
                        );
                      })}
            ...
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                {!dashboard?.velocity || dashboard.velocity.length === 0 ? (
                  <Typography sx={{ fontSize: 12, color: 'text.disabled', py: 3, textAlign: 'center' }}>No executions in the last 14 days</Typography>
                ) : (
                  <>
                    <MiniBarChart
                      data={dashboard.velocity}
                      valueKey="executions"
                      labelKey="exec_date"
                      color="#225038"
                      height={90}
                    ...
                    <Box sx={{ display: 'flex', gap: 3 }}>
                      <Box>
                        ...
                        <Typography sx={{ fontSize: 18, fontWeight: 800, color: '#225038' }}>
                          {dashboard.velocity.reduce((s, d) => s + parseInt(d.executions, 10), 0)}
                        </Typography>
                      </Box>
                      <Box>
                        <Typography sx={{ fontSize: 11, color: 'text.disabled' }}>Daily avg</Typography>
                        <Typography sx={{ fontSize: 18, fontWeight: 800, color: '#2563eb' }}>
                          {Math.round(dashboard.velocity.reduce((s, d) => s + parseInt(d.executions, 10), 0) / 14)}
                        </Typography>
                      </Box>
                      <Box>
                        <Typography sx={{ fontSize: 11, color: 'text.disabled' }}>Active days</Typography>
                        <Typography sx={{ fontSize: 18, fontWeight: 800, color: '#7c3aed' }}>
        ...
        <Grid container spacing={3} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12} md={6}>
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                <SectionTitle sub="Test coverage & pass rate by priority level">Coverage Heatmap</SectionTitle>
              ...
              <CardContent sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  ...
                  <Box sx={{ textAlign: 'right' }}>
                    <Typography sx={{ fontSize: 28, fontWeight: 800, color: debt.score >= 80 ? '#16a34a' : debt.score >= 50 ? '#d97706' : '#dc2626', lineHeight: 1 }}>
                      {debt.score ?? '—'}
                    </Typography>
                    <Typography sx={{ fontSize: 10, color: 'text.disabled', fontWeight: 600 }}>HEALTH SCORE</Typography>
                  </Box>
                </Box>
                {/* Health bar */}
                <Box sx={{ mb: 3 }}>
                  <Box sx={{ display: 'flex', height: 10, borderRadius: 5, bgcolor: '#f1f5f9', overflow: 'hidden', mb: 0.75 }}>
                    <Box sx={{ width: `${debt.score || 0}%`, bgcolor: debt.score >= 80 ? '#16a34a' : debt.score >= 50 ? '#d97706' : '#dc2626', borderRadius: 5, transition: 'width 0.6s' }} />
                  </Box>
                ...
                {[
                  { label: 'Never Run', value: debt.never_run || 0, desc: 'test cases with no execution history', icon: '⏸', color: '#dc2626', bg: '#fee2e2' },
                  { label: 'Missing Expected Result', value: debt.no_expected_result || 0, desc: 'test cases with no expected outcome defined', icon: '📝', color: '#d97706', bg: '#fef3c7' },
                  { label: 'Draft Status', value: rd.draft_count || 0, desc: 'test cases still in Draft state', icon: '✏️', color: '#2563eb', bg: '#dbeafe' },
                ].map(item => (
                  <Box key={item.label} sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1.75, '&:last-child': { mb: 0 } }}>
                    <Box sx={{ width: 36, height: 36, borderRadius: '9px', bgcolor: item.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 18 }}>
                      {item.icon}
                    </Box>
                    <Box sx={{ flex: 1 }}>
                      <Typography sx={{ fontSize: 12, fontWeight: 600, color: 'text.primary' }}>{item.label}</Typography>
                    ...
                    <Typography sx={{ fontSize: 20, fontWeight: 800, color: item.value === 0 ? '#16a34a' : item.color }}>
                      {item.value}
                    </Typography>
                  </Box>
                ))}
        ...
        <Grid container spacing={3} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12} md={6}>
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  <SectionTitle sub="Tests alternating between pass & fail across runs">Flaky Test Radar</SectionTitle>
                ...
                {!dashboard?.flaky_tests || dashboard.flaky_tests.length === 0 ? (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
                    <CheckCircleOutlineIcon sx={{ fontSize: 36, color: '#86efac', mb: 1 }} />
                    <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#16a34a' }}>No flaky tests detected</Typography>
                  ...
                  return (
                    <Box key={tc.id} sx={{ mb: 1.5, pb: 1.5, borderBottom: i < dashboard.flaky_tests.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.75 }}>
                        ...
                        <Chip label={`${failRate}% fail`} size="small"
                          sx={{ height: 18, fontSize: 10, fontWeight: 700, bgcolor: '#fee2e2', color: '#dc2626', borderRadius: '4px', flexShrink: 0 }} />
                      </Box>
                      <Box sx={{ display: 'flex', height: 5, borderRadius: 3, bgcolor: '#f1f5f9', overflow: 'hidden' }}>
            ...
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', height: '100%' }}>
              <CardContent sx={{ p: 3 }}>
                ...
                {!dashboard?.top_failing || dashboard.top_failing.length === 0 ? (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
                    <CheckCircleOutlineIcon sx={{ fontSize: 36, color: '#86efac', mb: 1 }} />
                    <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#16a34a' }}>No failures recorded</Typography>
                  ...
                  return (
                    <Box key={tc.id} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5, pb: 1.5, borderBottom: i < dashboard.top_failing.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                      <Box sx={{ width: 26, height: 26, borderRadius: '7px', bgcolor: '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Typography sx={{ fontSize: 12, fontWeight: 800, color: '#dc2626' }}>#{i + 1}</Typography>
                      </Box>
            ...
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white' }}>
              <CardContent sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                  <Box>
                    <Typography sx={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.6, color: 'text.secondary' }}>Automation Coverage</Typography>
                    <Typography sx={{ fontSize: 11, color: 'text.disabled', mt: 0.25 }}>How many test cases are automated vs pending — across all projects</Typography>
                    <Button size="small" onClick={refreshAutomation} sx={{ mt: 0.75, fontSize: 11, color: '#225038', fontWeight: 600, p: 0, minWidth: 0, '&:hover': { bgcolor: 'transparent', textDecoration: 'underline' } }}>↻ Refresh</Button>
                  </Box>
                  {/* Global totals */}
                  <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                    {[
                      ...
                      { label: 'Has Tool Assigned', value: dashboard?.automation?.typed ?? 0, color: '#7c3aed', bg: '#f3e8ff' },
                    ].map(s => (
                      <Box key={s.label} sx={{ textAlign: 'center', px: 2.5, py: 1.25, borderRadius: '12px', bgcolor: s.bg, border: '1px solid', borderColor: s.bg }}>
                    ...
                    {/* Big gauge */}
                    <Box sx={{ textAlign: 'center', px: 2.5, py: 1.25, borderRadius: '12px', bgcolor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                      <Typography sx={{ fontSize: 24, fontWeight: 800, color: (dashboard?.automation?.pct ?? 0) >= 70 ? '#16a34a' : (dashboard?.automation?.pct ?? 0) >= 40 ? '#d97706' : '#dc2626', lineHeight: 1 }}>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography sx={{ fontSize: 11, color: '#16a34a', fontWeight: 600 }}>✓ {dashboard?.automation?.automated ?? 0} automated</Typography>
                    <Typography sx={{ fontSize: 11, color: '#dc2626', fontWeight: 600 }}>{dashboard?.automation?.pending ?? 0} pending ✗</Typography>
                  </Box>
                </Box>
                ...
                {(dashboard?.automation?.by_project || []).filter(p => parseInt(p.total, 10) > 0).length > 0 ? (
                  <>
                    <Typography sx={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: 'text.disabled', mb: 1.5 }}>
                      Per Project
                    </Typography>
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: 'repeat(3, 1fr)', lg: 'repeat(4, 1fr)' }, gap: 2 }}>
                      {(dashboard?.automation?.by_project || []).filter(p => parseInt(p.total, 10) > 0).map(p => {
                        ...
                        const health = pct >= 70 ? { color: '#16a34a', bg: '#f0fdf4', border: '#86efac' }
                          : pct >= 40 ? { color: '#d97706', bg: '#fffbeb', border: '#fde047' }
                          : { color: '#dc2626', bg: '#fff1f2', border: '#fca5a5' };
                        return (
                          <Box key={p.id} sx={{ p: 2, borderRadius: '12px', border: '1.5px solid', borderColor: health.border, bgcolor: health.bg }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                              <Typography sx={{ fontSize: 12, fontWeight: 700, color: 'text.primary', flex: 1 }} noWrap>{p.name}</Typography>
                              <Typography sx={{ fontSize: 16, fontWeight: 800, color: health.color, ml: 1, flexShrink: 0 }}>{pct}%</Typography>
                            </Box>
                            {/* Mini stacked bar */}
                            <Box sx={{ display: 'flex', height: 8, borderRadius: 4, overflow: 'hidden', bgcolor: '#fee2e2', mb: 1 }}>
                              <Box sx={{ width: `${pct}%`, bgcolor: health.color, borderRadius: 4, transition: 'width 0.6s' }} />
                            </Box>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                              <Typography sx={{ fontSize: 10, color: '#16a34a', fontWeight: 600 }}>✓ {automated} auto</Typography>
                              <Typography sx={{ fontSize: 10, color: '#94a3b8' }}>{total} total</Typography>
                              <Typography sx={{ fontSize: 10, color: '#dc2626', fontWeight: 600 }}>{pending} pending</Typography>
                            </Box>
                          </Box>
                        );
                      })}
                  ...
                  <Box sx={{ textAlign: 'center', py: 3, bgcolor: '#f8fafc', borderRadius: '12px' }}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={8}>
            <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white', overflow: 'hidden' }}>
              <Box sx={{ px: 3, py: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid', borderColor: 'divider' }}>
                <Box>
                  <Typography sx={{ fontWeight: 700, fontSize: 15 }}>Test Runs</Typography>
                  <Typography sx={{ fontSize: 12, color: 'text.disabled' }}>{testRuns.length} total · {testRuns.filter(r => (r.computed_status || r.status) === 'Completed').length} completed</Typography>
                </Box>
                <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                  ...
                  <Button component={RouterLink} to="/test-runs" size="small" endIcon={<ArrowForwardIosIcon sx={{ fontSize: 11 }} />}
                    sx={{ fontSize: 12, fontWeight: 600, color: '#225038', whiteSpace: 'nowrap' }}>View all</Button>
                </Box>
              </Box>
              {testRuns.length === 0 ? (
                <Box sx={{ py: 8, textAlign: 'center' }}>
                  <PlayCircleOutlineIcon sx={{ fontSize: 48, color: '#cbd5e1', mb: 1.5 }} />
                  <Typography fontWeight={600} color="text.secondary" sx={{ mb: 2 }}>No test runs yet</Typography>
                  <Button component={RouterLink} to="/test-runs" variant="contained" startIcon={<AddIcon />}
                ...
                return (
                  <Box key={run.id} onClick={() => setSelectedRunId(isSelected ? '' : String(run.id))}
                    sx={{ px: 3, py: 2.25, borderBottom: idx < Math.min(testRuns.length, 8) - 1 ? '1px solid' : 'none', borderColor: 'divider',
                      bgcolor: isSelected ? 'rgba(34,80,56,0.03)' : 'transparent', borderLeft: isSelected ? '3px solid #225038' : '3px solid transparent',
                      cursor: 'pointer', transition: 'background 0.15s', '&:hover': { bgcolor: '#f8fafc' },
                      display: 'flex', alignItems: 'center', gap: 2.5 }}>
                    <Box sx={{ minWidth: 0, flex: '1 1 160px' }}>
                      <Typography sx={{ fontSize: 14, fontWeight: 600, color: 'text.primary', mb: 0.25 }} noWrap>{run.name}</Typography>
                    ...
                    <Box sx={{ flex: '0 0 120px', display: { xs: 'none', sm: 'block' } }}>
                      <Box sx={{ display: 'flex', height: 6, borderRadius: '10px', overflow: 'hidden', bgcolor: '#f1f5f9' }}>
                        {rTotal > 0 ? (<><Box sx={{ width: `${(rPass/rTotal)*100}%`, bgcolor: '#16a34a' }} /><Box sx={{ width: `${(rFail/rTotal)*100}%`, bgcolor: '#dc2626' }} /><Box sx={{ width: `${(rSkip/rTotal)*100}%`, bgcolor: '#f59e0b' }} /></>) : <Box sx={{ flex: 1, bgcolor: '#e2e8f0' }} />}
                      </Box>
                    ...
                    <Box sx={{ flex: '0 0 52px', textAlign: 'right', display: { xs: 'none', md: 'block' } }}>
                      <Typography sx={{ fontSize: 15, fontWeight: 800, color: rExec === 0 ? '#94a3b8' : rRate >= 80 ? '#16a34a' : rRate >= 50 ? '#d97706' : '#dc2626' }}>
                        {rExec === 0 ? '—' : `${rRate}%`}
                      </Typography>
                    </Box>
                    ...
                    <Button component={RouterLink} to={`/test-run/${run.id}`} size="small" variant={isSelected ? 'contained' : 'outlined'}
                      ...
                      sx={{ flexShrink: 0, fontSize: 12, fontWeight: 600, borderRadius: '8px', px: 1.75, py: 0.5, minWidth: 'unset',
                        ...(isSelected ? { bgcolor: '#225038', borderColor: '#225038', color: 'white', '&:hover': { bgcolor: '#1a3d2b' } }
          ...
          <Grid item xs={12} md={4}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              ...
              <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white' }}>
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mb: 2.5 }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      {[{ label: 'Passed', count: passed, color: '#16a34a', bg: '#dcfce7' }, { label: 'Failed', count: failed, color: '#dc2626', bg: '#fee2e2' }, { label: 'Skipped', count: skipped, color: '#d97706', bg: '#fef3c7' }].map(item => (
              ...
              <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white' }}>
                <CardContent sx={{ p: 2.5 }}>
                  ...
                  {(dashboard?.health_by_status || []).map(item => {
                    const statusColor = item.status === 'Active' ? { color: '#16a34a', bg: '#dcfce7' } : item.status === 'Deprecated' ? { color: '#dc2626', bg: '#fee2e2' } : { color: '#2563eb', bg: '#dbeafe' };
                    ...
                    return (
                      <Box key={item.status} sx={{ mb: 1.5, '&:last-child': { mb: 0 } }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                          <Typography sx={{ fontSize: 12, fontWeight: 600, color: statusColor.color }}>{item.status || 'Draft'}</Typography>
                          <Typography sx={{ fontSize: 12, fontWeight: 700, color: statusColor.color }}>{item.count} ({pct}%)</Typography>
                        </Box>
              ...
              <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '16px', bgcolor: 'white' }}>
                <CardContent sx={{ p: 2.5 }}>
                  <Typography sx={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: 'text.secondary', mb: 2 }}>Quick Actions</Typography>
                  {[
                    ...
                    <Box key={action.label} component={RouterLink} to={action.href}
                      sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.25, borderBottom: '1px solid #f1f5f9', textDecoration: 'none',
                        '&:last-child': { borderBottom: 'none', pb: 0 }, '&:hover .qa-label': { color: action.color }, transition: 'all 0.15s' }}>
                      <Box>
                        <Typography className="qa-label" sx={{ fontSize: 13, fontWeight: 600, color: 'text.primary', transition: 'color 0.15s' }}>{action.label}</Typography>
